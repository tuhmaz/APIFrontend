...
export const metadata: Metadata = {
  title: 'بحث عن الكلمات المفتاحية | المقالات',
  description: 'ابحث عن المقالات باستخدام الكلمات المفتاحية',
};
...
async function getPopularKeywords(countryCode: string) {
  try {
    const response = await apiClient.get<any>(
      ...
      { database: countryCode, per_page: 50, type: 'post' }
    );
    return response.data?.data?.post_keywords?.data || [];
  } catch (err) {
...
export default async function KeywordsIndexPage({ params }: Props) {
  ...
  let keywords = [];
  try {
     keywords = await getPopularKeywords(countryCode);
  } catch (error) {
     console.error(error);
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-16">
      <ClassHeader title="الكلمات المفتاحية للمقالات" />
        ...
        <div className="max-w-2xl mx-auto text-center">
          <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 mb-8">
            ...
            <p className="text-gray-500 mb-8">
              اكتب كلمة مفتاحية للبحث عن جميع المقالات المرتبطة بها.
            </p>
            ...
            <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100">
              ...
              <div className="flex flex-wrap justify-center gap-3">
                {keywords.map((k: any) => (
                  <Link
                    key={k.id}
                    href={`/${countryCode}/posts/keyword/${encodeURIComponent(k.keyword)}`}
